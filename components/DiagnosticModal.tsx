...
const ResultView = ({ result, onRestart }: any) => {
    ...
    return (
        <div className="p-8">
            <h3 className="text-xl font-black text-white mb-6 uppercase italic tracking-tighter">Diagnóstico Finalizado</h3>
            ...
            <div className="mt-8 flex gap-4">
                <a href="https://wa.me/13997744720" className="flex-1 bg-red-600 text-white py-4 rounded-xl font-black text-xs text-center uppercase tracking-widest hover:bg-red-700 transition-all">Iniciar Implementação</a>
                <button onClick={onRestart} className="flex-1 border border-white/10 text-white py-4 rounded-xl font-black text-xs uppercase hover:bg-white/5 transition-all">Nova Análise</button>
            </div>
        </div>
    );
};
...
const DiagnosticModal: React.FC<{ isOpen: boolean; onClose: () => void }> = ({ isOpen, onClose }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!process.env.API_KEY) {
            setError("Chave de API não configurada corretamente.");
            return;
        }
        ...
        const prompt = `Analise os seguintes dados de negócio para o Grupo CBL e dê um diagnóstico tático: ${JSON.stringify(formData)}.`;
        ...
        try {
            ...
            setError("Falha na análise. Verifique sua chave de API.");
            setView('form');
        }
    };
                ...
                {view === 'form' && (
                    <form onSubmit={handleSubmit} className="p-8 space-y-6">
                        <div className="text-center">
                            <h2 className="text-2xl font-black text-white uppercase italic tracking-tighter">Raio-X de Oportunidades</h2>
                            <p className="text-gray-500 text-[10px] uppercase font-bold tracking-[0.2em] mt-1">Engenharia Estratégica Grupo CBL</p>
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-h-[50vh] overflow-y-auto pr-2 custom-scrollbar">
                            ...
                            <FormField label="Ads/Mês" name="investimento" type="number" value={formData.investimento} onChange={(e:any) => setFormData({...formData, investimento: e.target.value})} onCheckboxChange={(e:any) => setCheckboxes({...checkboxes, investimento: e.target.checked})} isChecked={checkboxes.investimento} />
                        </div>
